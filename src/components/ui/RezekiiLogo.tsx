...
export default function RezekiiLogo({ size = "md", variant = "light", iconOnly = false }: RezekiiLogoProps) {
  ...
  return (
    <div className={`inline-flex items-center ${gap}`}>
      {/* Icon mark: green rounded square with white R + warm gradient arrow accent */}
      <svg
        width={iconSize}
        height={iconSize}
      ...
      >
        ...
        {/* Warm gradient arrow accent — top-right */}
        <circle
          cx={iconSize * 0.78}
          cy={iconSize * 0.22}
          r={iconSize * 0.13}
        ...
        />
        <path
          d={`M ${iconSize * 0.74} ${iconSize * 0.18} L ${iconSize * 0.82} ${iconSize * 0.18} L ${iconSize * 0.82} ${iconSize * 0.26}`}
          stroke="white"
